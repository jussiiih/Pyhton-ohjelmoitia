...
def eniten_maaleja (lista: list):
    return (max(lista, key=lambda alkio: alkio.maalit)).nimi

def eniten_pisteita (lista:list):
    eniten =(max(lista, key=lambda alkio: alkio.maalit + alkio.syotot))
    return (eniten.nimi, eniten.pelinumero)

def vahiten_minuutteja (lista:list):
    return min(lista, key=lambda alkio: alkio.minuutit)
